const express = require('express');
const cors = require('cors');
require('dotenv').config();

const app = express();

// Middleware
app.use(cors());
app.use(express.json());

// Routes
const fixturesRoutes = require('./routes/fixtures');
const nbaRoutes = require('./routes/basketball');
const footballRoutes = require('./routes/football');
const tennisRoutes = require('./routes/tennisRoutes');
const motorsportRoutes = require('./routes/motorsportRoutes');
const hockeyController = require('./controllers/hockeyController');
const baseballController = require('./controllers/baseballController');
const nflController = require('./controllers/nflController');
const sportsRoutes = require('./routes/sportsRoutes');
const streamProxy = require('./routes/streamProxy');
const streamsRoutes = require('./routes/streams');

app.use('/api/fixtures', fixturesRoutes);
app.use('/api/basketball', nbaRoutes);
app.use('/api/football', footballRoutes);
app.use('/api/tennis', tennisRoutes);
app.use('/api/motorsport', motorsportRoutes);
app.get('/api/hockey', hockeyController.getHockeyMatches);
app.get('/api/hockey/stream/:streamId', hockeyController.getHockeyStreamInfo);
app.get('/api/baseball', baseballController.getBaseballMatches);
app.get('/api/baseball/stream/:streamId', baseballController.getBaseballStreamInfo);
app.get('/api/nfl', nflController.getNFLMatches);
app.get('/api/nfl/stream/:streamId', nflController.getNFLStreamInfo);
app.use('/api/sports', sportsRoutes);
app.use('/api/stream', streamProxy);
app.use('/api/streams', streamsRoutes);


// Health check
app.get('/', (req, res) => {
    res.json({
        status: 'ok',
        message: 'SportMeriah API',
        endpoints: {
            fixtures: '/api/fixtures/today',
            basketball: '/api/basketball',
            stream: '/api/stream/:streamId.m3u8'
        }
    });
});

// Error handling
app.use((err, req, res, next) => {
    console.error('Server Error:', err);
    res.status(500).json({ error: 'Internal Server Error' });
});

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => {
    console.log(`Server running on port ${PORT}`);
});
